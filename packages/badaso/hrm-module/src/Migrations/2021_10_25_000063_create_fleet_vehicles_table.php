...
class CreateFleetVehiclesTable extends Migration
{
    ...
    public function up()
    {
        Schema::create(config('badaso.database.prefix').'fleet_vehicles', function (Blueprint $table) {
            $table->id();
            ...
            $table->foreign('fleet_state_id')->references('id')->on(config('badaso.database.prefix').'fleet_states')->onDelete('cascade');
            $table->foreign('driver_partner_id')->references('id')->on(config('badaso.database.prefix').'partners')->onDelete('cascade');
            $table->foreign('future_driver_partner_id')->references('id')->on(config('badaso.database.prefix').'partners')->onDelete('cascade');
    ...
    {
        Schema::dropIfExists(config('badaso.database.prefix').'fleet_vehicles');
    }
}
